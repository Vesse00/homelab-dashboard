...
import { X, Search, LayoutGrid, Box, Plus, Activity, CloudSun, Cpu, Clock, Server, MonitorSmartphone, Wifi, PlaySquare, Cloud, ShieldCheck, Gamepad2 } from 'lucide-react';
...
// ============================================================================
// Zoptymalizowany Kafelek
// ============================================================================
const GalleryItem = React.memo(({ widget, theme, onAddWidget, onAddService, onClose, t }: any) => {
  ...
  return (
    ...
    >
       ...
       <h3 className="text-white font-bold text-lg mb-1 truncate pr-2 tracking-tight group-hover:text-purple-100 transition-colors duration-300 ease-out">
         {widget.name}
       </h3>
       ...
       <button 
         onClick={() => {
           if (widget.isService && widget.data) {
             onAddService ? onAddService(widget.data) : onAddWidget(`kiosk-${widget.data.widgetType}`, widget.data);
           } else {
             onAddWidget(widget.id);
           }
           onClose(); 
         }}
         className={`w-full py-2.5 rounded-xl text-sm font-bold flex items-center justify-center gap-2 transition-all duration-300 ease-out bg-slate-900/50 hover:bg-purple-600 text-slate-300 hover:text-white border border-slate-700/50 hover:border-purple-500/50 active:scale-95 will-change-transform`}
       >
...
export default function KioskWidgetGalleryModal({ isOpen, onClose, onAddWidget, onAddService }: any) {
  ...
  // Używamy tych samych kategorii co w głównej galerii!
  ...
  const allWidgets = useMemo<WidgetItem[]>(() => {
    // 1. Podstawowe widgety kiosku (zawsze widoczne w kategorii system/media)
    const core: WidgetItem[] = [
      { id: 'kiosk-system', name: 'Statystyki Systemu', desc: 'Wielki podgląd CPU i RAM', icon: Cpu, category: 'system', color: 'emerald', isService: false },
      { id: 'kiosk-weather', name: 'Pogoda', desc: 'Wielkoformatowa prognoza pogody', icon: CloudSun, category: 'media', color: 'yellow', isService: false },
      { id: 'kiosk-clock', name: 'Zegar Ścienny', desc: 'Minimalistyczny zegar z datą', icon: Clock, category: 'system', color: 'blue', isService: false },
    ];

    // 2. Mapowanie zeskanowanych usług do konkretnych kategorii na podstawie KNOWN_APPS!
    const services: WidgetItem[] = dbServices.map((s: any) => {
      ...
      return {
        ...
        data: { 
          ...
          color: knownApp?.color || 'purple',
          status: 'running',
          widgetType: `kiosk-${s.type}`, // Prefix kiosku
        },
        id: s.id,
        name: s.name,
        ...
        category: knownApp?.category || 'apps', // <--- MAGIA! Rozpoznaje kategorię automatycznie
        color: knownApp?.color || 'purple' // Pobiera oryginalny kolor z klasycznej galerii
      };
    });
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-slate-950/80 backdrop-blur-md p-4 sm:p-6 transition-opacity duration-300">
      <div className="bg-slate-900/95 border border-purple-500/30 rounded-2xl w-full max-w-6xl h-[85vh] shadow-[0_0_50px_rgba(168,85,247,0.15)] flex overflow-hidden backdrop-blur-xl animate-in fade-in zoom-in-95 duration-300 ease-out">
        ...
        <div className="w-64 bg-slate-950/40 border-r border-slate-800/50 hidden md:flex flex-col">
          <div className="p-6 border-b border-slate-800/50">
            <h2 className="text-xl font-bold text-white flex items-center gap-2 tracking-tight">
              <MonitorSmartphone className="text-purple-400" /> Kiosk Gallery
            </h2>
            <p className="text-xs text-slate-500 mt-2 leading-relaxed">Widgety zoptymalizowane pod ekrany dotykowe (ściana/tablet).</p>
          </div>
          <div className="p-4 flex-1 space-y-1 overflow-y-auto custom-scrollbar">
            {visibleCategories.map(cat => (
                <button key={cat.id} onClick={() => setActiveCategory(cat.id)} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-300 ease-out text-sm font-medium ${activeCategory === cat.id ? 'bg-purple-600 text-white shadow-lg shadow-purple-500/20 scale-[1.02]' : 'text-slate-400 hover:text-slate-200 hover:bg-white/5'}`}>
        ...
        {/* Prawa Część: Wyszukiwarka i Grid */}
        <div className="flex-1 flex flex-col h-full overflow-hidden relative bg-gradient-to-br from-slate-900/50 to-slate-950/50">
          <div className="p-6 border-b border-slate-800/50 flex items-center justify-between gap-4">
             <div className="relative w-full max-w-md group">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within:text-purple-400 transition-colors duration-300" size={18} />
                <input 
                  type="text" placeholder={t('searchPlaceholder')} 
                  value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
                  className="w-full bg-black/20 border border-slate-700/50 rounded-xl py-2.5 pl-10 pr-4 text-sm text-white placeholder-slate-500 focus:border-purple-500/50 focus:outline-none focus:ring-2 focus:ring-purple-500/20 transition-all duration-300"
                />
             </div>
