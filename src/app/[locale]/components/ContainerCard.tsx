...
export default function ContainerCard({ container }: Props) {
  const t = useTranslations('Widgets.ContainerCard');
      ...
      .then((data) => {
        if (Array.isArray(data)) {
          const chartData = data.map((item: any) => ({
            ...item,
            numericState: item.state === 'running' ? 1 : 0,
  ...
  return (
    <div className={`
      ...
      ${isRunning ? 'bg-slate-800/40 border-slate-700/50 hover:border-emerald-500/50' : 'bg-red-900/10 border-red-900/30 hover:border-red-500/50'}
    `}>
      <div className="flex justify-between items-start mb-2 z-10 relative drag-handle cursor-move">
        ...
        <div className={`shrink-0 w-2.5 h-2.5 rounded-full shadow-[0_0_8px_rgba(0,0,0,0.5)] mt-1.5 ${isRunning ? 'bg-emerald-400 shadow-emerald-500/50' : 'bg-red-500 shadow-red-500/50'}`} />
      </div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={history}>
            ...
            <Tooltip 
              ...
              formatter={(value: any) => [value === 1 ? t('online') : t('offline'), t('status')]}
            />
            <Line type="step" dataKey="numericState" stroke={isRunning ? '#10b981' : '#ef4444'} strokeWidth={2} dot={false} isAnimationActive={false} />
          </LineChart>
        </ResponsiveContainer>
      </div>
